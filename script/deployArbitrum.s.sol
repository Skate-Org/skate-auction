...
contract DeployScript is Script {
    function run() external {
        console2.log('========= ARB SEPOLIA DEPLOYMENTS =========');
        vm.startBroadcast();

        ExecutorRegistry executorRegistryArb = new ExecutorRegistry();
        executorRegistryArb.addExecutor(
            address(0x00E3CC76DA72cF673E4429E2d527388956af61ef)
        );
        console2.log('ExecutorRegistryArb: ', address(executorRegistryArb));
        ISkateGateway gatewayArb = new SkateGateway();
        console2.log('Gateway: ', address(gatewayArb));
        address impl = address(new SkateAuctionPeriphery());

        address auctionTokenArb =
            address(new SkateAuctionToken('AuctionToken', 'AT'));
        console2.log('Token: ', address(auctionTokenArb));
        ISkateAuctionPeriphery auctionPeripheryArb = SkateAuctionPeriphery(
            address(
                new ERC1967Proxy(
                    impl,
        ...
        console2.log('SkateNFTPeriphery: ', address(auctionPeripheryArb));
        vm.stopBroadcast();
    }
}
